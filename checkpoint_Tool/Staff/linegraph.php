   ...
   include_once '../Config/cleandata.php';
       ...
       try
      {
        if (isset($_POST['submit']))
        {
            ...
            $check = "SELECT tool.tableNamex FROM data join tool on tool.toolID = data.toolID join students on students.studentID = data.studentID where data.studentID = '$studentnum' AND tool.tableNamex = 'Difficulty' GROUP BY data.studentID";
            $ck = $pdo->query($check);
            $countRows =  $ck->rowCount(); 

            
            //second check to see if there is data in the Y field
            $check2 = "SELECT tool.tableNamey FROM data join tool on tool.toolID = data.toolID join students on students.studentID = data.studentID where data.studentID = '$studentnum' AND tool.tableNamey = 'Interest' GROUP BY data.studentID";
