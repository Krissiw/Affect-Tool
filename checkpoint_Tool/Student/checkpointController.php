<!--Designed By: Olufemi Olusina & Albert Warner
    ...
    Description: 
    This PHP file is the controller for the checkpoint mark off function.  
...
<?php 
    ...
    ###############################################
    # N.B: Go to Line 125 to change the passcode. #
    ###############################################
    ...
    *****************************************/
    ...
    if(isset($_POST['checkpointComplete'])) {
        ...
        ###########################################
        $multiplier = $labNumber * 2; //Takes the lab number selected multiplied by 2 and concatenated to create password.
        $passCode = $labNumber."2".$multiplier;//sets the password
        ...
        $success = true;
            ...
            if ($password == $passCode) {
                ...
                // Check if student has entry in completion table, and has used the grid
                $stmt = $pdo->prepare("SELECT COUNT(*) AS `total` FROM completion WHERE studentID = :studentID AND labID = :labID");
                $stmt->execute(array(':studentID' => $studentID,':labID' => $labNumber));
                $result = $stmt->fetchObject();
                    
                if ($result->total > 0) 
                {
                    $message = "Sorry...you have already been marked off for this lab!.";
                    include "alreadyMarked.html.php";
                }
                else {
                    ...
                    if ($result->total > 0) 
                    {
                        try{
                            ...
                            $insertQuery ="INSERT into completion(studentID,labID,responseTime)                
                            ...
                            $message = "You have been marked off for checkpoint completed!.";
    ...
    /************************************************************************
    *Else if any other button is triggered other than mark checkpoint button
    *Load the checkpoint form page
    *************************************************************************/
   else {
        include "checkpointForm.html.php";
   } 
